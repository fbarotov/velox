...
cudf::ast::expression const& AstContext::multipleInputsToPairWise(
  ...
  auto result = &pushExprToTree(call->inputs()[0]);
...
cudf::ast::expression const& AstContext::pushExprToTree(
  ...
  switch (expr->kind()) {
    ...
    case core::ExprKind::kCall: {
      ...
      if (binaryOps.find(name) != binaryOps.end()) {
        if (name == "and" or name == "or") {
          const auto* call = expr->asUnchecked<core::CallTypedExpr>();
          return multipleInputsToPairWise(call);
        }
      ...
      // Fall through for unrecognized call names
    }
    ...
    // TODO(ps): Select evaluator outside of Ast expression.
    // Fallback: delegate to another evaluator.
    default: {
      const auto* evaluator = findBestEvaluator(expr, kAstEvaluatorName);
      if (evaluator != nullptr) {
        int sideIdx = findExpressionSide(expr);
        if (sideIdx < 0) {
          sideIdx = 0;
        }
        auto node = evaluator->create(expr, inputRowSchema[sideIdx]);
        return addPrecomputeInstructionOnSide(
            sideIdx, 0, expr->toString(), "", node);
      }
      VELOX_FAIL("Unsupported expression: {}", expr->toString());
    }
  }
}
